...
#define CAN_DEV_NEOPIXEL    1
#define CAN_DEV_SERVOMOTOR  2
...
bool canOpen(uint8_t ch, CanMode_t mode, CanFrame_t frame, CanBaud_t baud, CanBaud_t baud_data)
{
    ...
    case _DEF_CAN1:
    	...
    	can_tbl[ch].enable_int            = FDCAN_IT_LIST_RX_FIFO0 | FDCAN_IT_BUS_OFF | FDCAN_IT_ERROR_WARNING | FDCAN_IT_ERROR_PASSIVE;
...
void canAutoRead(void)
{
  uint32_t index = 0;
  //  m2s >> ch 또는
  if (canMsgAvailable(m2s))
  {
    can_msg_t msg;

    canMsgRead(m2s, &msg);

    uint32_t ch = (uint32_t)(msg.data[1]);
    ...
    switch(msg.data[0])
    {
      case CAN_DEV_NEOPIXEL:
        //  data[0] = index
        //  data[1] = red
        //  data[2] = green
        //  data[3] = blue
        ws2812SetColor(ch, msg.data[2],msg.data[3],msg.data[4],msg.data[5]);
        break;

      case CAN_DEV_SERVOMOTOR:
        //  data[0] = angle_low
        //  data[1] = angle_high
        int16_t angle = msg.data[2] + (msg.data[3] << 8);
        servoSet(ch, angle);
        break;
    }
#if 0
    index %= 1000;
    cliPrintf("ch %d %03d(R) <- id ",m2s, index++);
