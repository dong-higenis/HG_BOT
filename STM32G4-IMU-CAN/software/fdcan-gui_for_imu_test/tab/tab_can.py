...
class TabCAN(QWidget, Ui_CAN):
  def __init__(self, ui_main :Ui_MainWindow, cmd, config_item, syslog):
    ...
    self.cube_control = CubeControlWidget()
    layout.addWidget(self.cube_control)
    self.widget.setLayout(layout)

    self.cube_control.cube_widget.setRoll(120)
    self.cube_control.cube_widget.setYaw(120)
    self.cube_control.cube_widget.setPitch(120)
    self.cube_control.cube_widget.setScale(1)
  ...
  def convert_angle(self, angle):
    if angle < 0:
        return angle + 360
    return angle
  def updateImu(self, can_msg):
    if can_msg.id == 0x100 and can_msg.frame == CAN_CLASSIC:
      #imu id
      if can_msg.dlc == 8:
        count = can_msg.data[0]
        status = (can_msg.data[1] >> 1)
        button = (can_msg.data[1] & 0x01)
        roll  = self.convert_angle(np.int16(((can_msg.data[2] << 8) | can_msg.data[3])))
        pitch = self.convert_angle(np.int16(((can_msg.data[4] << 8) | can_msg.data[5])))
        yaw   = self.convert_angle(np.int16(((can_msg.data[6] << 8) | can_msg.data[7])))
        print(f'count: {count}, status: {status}, button: {button}, roll: {roll}, pitch: {pitch}, yaw: {yaw}')
        self.cube_control.cube_widget.setRoll(roll)
        self.cube_control.cube_widget.setYaw(yaw)
        self.cube_control.cube_widget.setPitch(pitch)    
